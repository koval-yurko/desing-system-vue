...
- Customize PrimeVue in this priority order (pick the first that fits, do not skip ahead):
  ...
  2. **`:pt="..."`** to attach your own class/attrs to a specific inner part (`header`, `overlay`, `pcCloseButton`, …) when tokens cannot reach the property. Style the attached class with plain scoped CSS — no `!important`. Reference: `DsModal.vue`, `DsSelect.vue`.
     - ⚠️ **Scope-id gotcha (verified Story 11.1):** plain scoped CSS only reaches a `:pt`-attached class on the **`root`** part. Vue applies the wrapper's `data-v-` scope id to its own template nodes and to a child component's **root** element only — **not** to PrimeVue's inner parts. So `:pt={ icon: { class: 'ds-x__icon' } }` + a plain `.ds-x__icon { … }` rule compiles to `.ds-x__icon[data-v-HASH]` and **silently matches nothing** (the rendered inner span carries no `data-v-HASH`). For **inner** parts (`label`, `icon`, `header`, `overlay`, …) you must reach across scope: `:deep(.ds-x__icon) { … }`, or `:global(...)`, or keep `:deep(.p-*)`. `:dt` (step 1) and `root`-part `:pt` are unaffected. Verify with a DOM probe / built-CSS check before retiring a `:deep(.p-*)` selector on a non-root part.
